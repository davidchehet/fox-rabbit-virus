...
public class Simulator
{
    ...
    public Simulator(int depth, int width)
    {
        ...
        //view.setColor(FoxRabbitProject.Rabbit.class, Color.ORANGE);
        //view.setColor(FoxRabbitProject.Fox.class, Color.BLUE);
        //view.setColor(FoxRabbitProject.Virus.class, Color.RED);
    ...
    public void simulate(int numSteps)
    {
        for(int step = 1; step <= numSteps && view.isViable(field); step++) {
            simulateOneStep();
            delay(120);   //uncomment for slower run
        }
    }
        ...
        for(int row = 0; row < field.getDepth(); row++) {
            for(int col = 0; col < field.getWidth(); col++) {
                if(rand.nextDouble() <= FOX_CREATION_PROBABILITY) {
                    ...
                    Location location = new Location(row, col);
                    Virus virus = new Virus(true, field, location);
                    animals.add(virus);
